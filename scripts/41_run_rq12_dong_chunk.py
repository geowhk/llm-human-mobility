...
def main() -> None:
    ...
    try:
        ...
        _log_progress(log_path, f"INPUT_PATHS_RESOLVED dong={dong_abs}")
        chunk_id = _extract_chunk_id(resolved_run_id, dong_abs.stem)
        if chunk_id is not None:
            write_text(str(log_path), f"CHUNK_ID: {chunk_id}\n")
            _log_progress(log_path, f"START chunk_id={chunk_id}")

        dong_df = load_parquet(str(dong_abs)).copy()
        _log_progress(log_path, f"DATA_LOADED dong_rows={len(dong_df)}")
        prompts_dong = build_prompts_df(dong_df, "dong", config)
        _log_progress(log_path, f"PROMPTS_BUILT dong_rows={len(prompts_dong)}")
